...
class TestIngestion(unittest.TestCase):
    ...
    # rtlsdr_multisonde_go version / type checks
    def test_good_rtlsdr_multisonde_payload(self):
        payload = copy.deepcopy(example_body)
        payload[0]["datetime"] = datetime.datetime.now(datetime.UTC).isoformat()
    ...
    def test_bad_rtlsdr_multisonde_version(self):
        payload = copy.deepcopy(example_body)
        payload[0]["datetime"] = datetime.datetime.now(datetime.UTC).isoformat()
        payload[0]["software_name"] = "rtlsdr_multisonde_go"
        payload[0]["software_version"] = "0.1.0"
        payload[0]["type"] = "RS41"
        output = lambda_handler(compress_payload(payload), fakeContext())
        sns.publish.assert_not_called()
        body_decode = json.loads(output["body"])
    ...
    def test_dfm09p_misid_payload(self):
        payload = copy.deepcopy(example_body)
        payload[0]["datetime"] = datetime.datetime.now(datetime.UTC).isoformat()
        payload[0]["type"] = "DFM"
        payload[0]["subtype"] = "DFM09P"
        payload[0]["serial"] = "23068595"
        output = lambda_handler(compress_payload(payload), fakeContext())
        sns.publish.assert_called()
        body_decode = json.loads(output["body"])
    ...
    def test_dropping_2_payload(self):
        ...
        payload.append(dict(payload[0]))
        payload[2]["datetime"] = (ref_time-datetime.timedelta(seconds=2)).isoformat()
        payload.append(dict(payload[0])) 
        payload[3]["datetime"] = (ref_time-datetime.timedelta(seconds=3)).isoformat()
        payload.append(dict(payload[0])) 
        payload[4]["datetime"] = (ref_time-datetime.timedelta(seconds=4)).isoformat()
        payload.append(dict(payload[0]))
        payload[5]["datetime"] = (ref_time-datetime.timedelta(seconds=5)).isoformat()
        print([x['datetime'] for x in payload])
        output = lambda_handler(compress_payload(payload), fakeContext())
        sns_call = json.loads(zlib.decompress(base64.b64decode(sns.publish.call_args_list[0][1]['Message']), 16 + zlib.MAX_WBITS))
        print([x['datetime'] for x in sns_call])
        self.assertIn(payload[0]["datetime"], [x['datetime'] for x in sns_call])
        self.assertNotIn(payload[1]["datetime"], [x['datetime'] for x in sns_call])
        self.assertIn(payload[2]["datetime"], [x['datetime'] for x in sns_call])
        self.assertNotIn(payload[3]["datetime"], [x['datetime'] for x in sns_call])
        self.assertIn(payload[4]["datetime"], [x['datetime'] for x in sns_call])
        self.assertIn(payload[5]["datetime"], [x['datetime'] for x in sns_call])
        self.assertEqual(len(sns_call), 4)
